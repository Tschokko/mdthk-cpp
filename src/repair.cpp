...
int repair_movie_list_json(std::istream& src, std::ostream& dst) {
  ...
  std::vector<mediathek::parser::movie_entry> movie_entries{};

  for (std::size_t i = 0; i < entries.size(); i++) {
    json j{entries[i]};
    if (j.is_array()) {
      std::vector<std::string> vals = j;
      movie_entries.push_back(mediathek::parser::movie_entry{vals});
    }
  }

  std::cout << "Entries: " << movie_entries.size() << std::endl;
